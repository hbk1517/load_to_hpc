...
public class TestStateless{
	public static void main(String args[]){
		...
		try{
			if(args.length!=6){
				System.out.println("Usage: java TestStateless mapperFile reducerFile inputFile outputFile keepTempFiles PythonVersionNumber");
				System.exit(0);
			}
			//split up the input
			...
			for(int i=0; i<fileNumber; i++){
				...
				if(args[5].equals("2")){
					...
					builder.command("python3.6", args[0]);
				}
			...
			for(int i=0; i<fileNumber; i++){
				...
				if(args[5].equals("2")){
                    ...
                    builder.command("python3.6", args[1]);
                }
				Process p = builder.start();
				OutputStream pos = p.getOutputStream();
				InputStream ios = p.getInputStream();
			...
			finalOutput.close();
			System.out.println("Mapping and Reducing completed successfully. Your output is stored in "+args[3]+". This message does not mean your answer is correct though. All this message is saying is that your mapper and reducer executed without failing. It does not mean the mapper and reducer are correct nor does it mean your code is stateless. Check your output. If the output is correct, your code is likely stateless.");
		} catch(Exception e){
